var PRESTimeline = /** @class */ (function () {
    ...
    PRESTimeline.prototype._generateTimeline = function () {
        ...
        for (var i = 0; i < numNode; i++) {
            ...
            var id = 'ball_' + i;
            var el = wrap.append('<li id ="' + id + '" class="' + this.cardData[i].period + '" style="border-color: ' + c + '"></li>');
        }
        // ## width of timeline
        var nodeW = 400;
    ...
    PRESTimeline.prototype._initialColor = function () {
        for (var i = 0; i < this.periodData.length; i++) {
            ...
            sbstyle.sheet.insertRule('li.' + p + '.active { background-color: #32ccf4 !important } ', 0);
            sbstyle.sheet.insertRule('li.' + p + '::before { background-color: #32ccf4 } ', 0);
            sbstyle.sheet.insertRule('li.' + p + '::after { background-color: #32ccf4 } ', 0);
        }
    ...
    PRESTimeline.prototype._adjustCardContainer = function () {
        var activeH = $(this.activeCard).outerHeight() + 340;
                ...
                if (this.activeCard.period != this.activePeriod.period) {
                    ...
                    for (var i = 0; i < this.periodData.length; i++) {
                        ...
                        if (this.activeCard.period === temp.period){
                            $(temp).css('border-color', '#32ccf4');
                            $(temp).find('.year').css('color', '#32ccf4');
                            document.getElementById(id_ball).style.borderColor = "#32ccf4";
                            
                            var p = this.cardData[i].period;
                            this.cardData[i].color = this.color[p];
                            var temp2 = this.cardData[i];
                            $(temp2).css('border-color', '#32ccf4');
       
                            ta = temp;
                        }
                        else{
                            $(temp).css('border-color', 'gray');
                            $(temp).find('.year').css('color', 'gray');
                            document.getElementById(id_ball).style.borderColor = "gray";
                            
                            var p = this.cardData[i].period;
                            this.cardData[i].color = this.color[p];
                            var temp2 = this.cardData[i];
                            $(temp2).css('border-color', 'gray');
                        }
                    }
